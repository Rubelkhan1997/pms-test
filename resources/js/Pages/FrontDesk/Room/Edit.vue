<template>
    <Head :title="t('rooms.edit_room')" />

    <div v-if="canEdit" class="max-w-4xl mx-auto">
        <section class="space-y-6">
            <div class="flex justify-between items-center">
                <div>
                    <h1 class="text-2xl font-semibold text-slate-800">{{ t('rooms.edit_room') }}</h1>
                    <p class="text-sm text-slate-500 mt-1">{{ t('rooms.update_hint') }}</p>
                </div>
...
<script setup lang="ts">
    ...
    import type { Room, RoomHotelOption, RoomStatus } from '@/Types/FrontDesk/room';
    import { mapRoomHotelOptionApi, mapToRoom } from '@/Utils/Mappers/room';
    ...
    const { update: updateRoom, saving } = useRooms();
    ...
    const form = useForm({
        ...
        baseRate: room.baseRate.toString() as number | string,
    });
