<template>
    <div :class="wrapperClass">
        <label v-if="label" :for="resolvedId"
            :class="['text-gray-800 font-medium mb-2', labelClass || 'text-[14px] block']">
            {{ label }}
            <span v-if="required" class="text-red-500">*</span>
        </label>

        <select :id="resolvedId" :name="name || resolvedId" :required="required" :disabled="disabled" :class="[
            ...
            inputClass,
        ]" :value="modelValue ?? ''" @change="onChange" @blur="$emit('blur', $event)">
            <option v-if="placeholder" value="">{{ placeholder }}</option>
            <option v-for="option in options" :key="String(option[optionValue])" :value="option[optionValue]">
                {{ option[optionLabel] }}
            </option>
        </select>
